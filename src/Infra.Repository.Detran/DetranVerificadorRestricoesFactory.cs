...
{
    public class DetranVerificadorRestricoesFactory : IDetranVerificadorRestricoesFactory
    {
        private readonly IServiceProvider _ServiceProvider;
        private readonly IDictionary<string, Type> _Repositories = new Dictionary<string, Type>();

        public DetranVerificadorRestricoesFactory(IServiceProvider serviceProvider)
        {
        ...
        {
            IDetranVerificadorRestricoesRepository result = null;

            if (_Repositories.TryGetValue(UF, out Type type))
            {
                result = _ServiceProvider.GetService(type) as IDetranVerificadorRestricoesRepository;
            }
